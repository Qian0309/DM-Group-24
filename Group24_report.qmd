...
The Conceptual Schema explained in Section 1.1 is converted into Logical Schema after normalising using the below steps,

1NF : Multi-valued attributes include 'Buyer_Address' in Buyer entity, 'Supplier_Address' in Supplier entity and 'Dimensions' in Product entity that must be normalised. To do the same, a separate Address table is created that stores Address information with attributes that inlcude address_id, house_number, house_name, street, city, county and post_code. The Address entity is then referenced to Buyer and Supplier entities using referential keys, i.e., address_id would be a foreign key in Supplier and Buyer entities with 1:1 cardinality. Dimension is created as a separate entity and referenced to Product table with 1:1 cardinality. 

2NF : Product and Buyer has a M:N relationship and hence violates 2NF having Functional Dependencies were many Products can bought by single Buyer and the vice versa creating duplicate entries of primary key attribute in each table. To overcome this, a relationship table called 'order_details' is created with product_id and buyer_id being compositely stored as primary key with 'order_id' primary key of order_details table.

3NF : The Logical Schema is designed to include only attributes that are directly dependent on primary key of their respective table with no transitive dependencies. 
